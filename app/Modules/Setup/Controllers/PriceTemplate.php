...
use App\Modules\Setup\Services\PriceTemplateServiceProvider as Services;
use App\Modules\Setup\Models\PriceTemplate as PriceTemplateModel;
use App\Modules\Setup\Models\PriceHistory;
use Request;
use Response;
use Permission;

class PriceTemplate extends Controller {
	private $media =
		[
	...
	private $url = [ 'page' => 'setup/hotel/price-template/','form'=> '.form_crud','autoDataTable' => false ];

	public $views = 'Setup.Views.PriceTemplate.';
				...
				case 'save':
					if ($this->permission->has('add')) {
						...
						if ($validation['error']) {
							...
							$this->model->create(assertCreated($data));
				...
				case 'update':
					if ($this->permission->has('edit')) {
						...
						if ($validation['error']) {
							...
							$data = $this->services->post(Request::all());
							if ($this->services->isUpdatedPrice(
									$this->model,
									decode(Request::get('id')),
									Request::get('amount')
								)
							) {
								$this->history->create(
									$this->services->getHistory(
										getRawData(
											$this->model->where('index_id',decode(Request::get('id')))->get()
										)
									)
								);
							}
							$this->model->where('index_id',decode(Request::get('id')))->update(assertCreated(assertModified($data)));
	...
	private function init($key = null) 
	{
		$this->initializer();
		return array(
			'table' => $key == null ?  $this->model->data()->get() : array()
		);
	}

	private function refreshTable()
	{
		return view($this->views.'tables.table',['table'=>$this->model->data()->get()])->render();
	}

	private function initializer()
	{
		$this->services = new Services;
		$this->model = new PriceTemplateModel;
		$this->history = new PriceHistory;
		$this->permission = new Permission('price-template');
	}
}