...
use App\Modules\Setup\Services\PriceTypeServiceProvider as Services;
use App\Modules\Setup\Models\PriceType as PriceTypeModel;
use Request;
use Response;
use Permission;
...
class PriceType extends Controller {
	...
	private $url = [ 'page' => 'setup/hotel/price-type/','form'=> '.form_crud' ];

	public $views = 'Setup.Views.PriceType.';
				...
				case 'update':
					if ($this->permission->has('edit')) {
						...
						if ($validation['error']) {
							...
							$data = $this->services->post(Request::all());
							$this->model->where('index_id',decode(Request::get('id')))->update(assertModified($data));
