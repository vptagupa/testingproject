...
use App\Modules\Setup\Services\RoomServiceProvider as Services;
use App\Modules\Setup\Models\Room as RoomModel;
use Request;
use Response;
use Permission;
...
class Room extends Controller {
	private $media =
		[
			...
			'Description'=> 'Manage',
			'js'		=> ['crud','setup/Room'],
			'init'		=> ['CRUD.init()','Room.init()','FN.dataTable()'],
			'plugin_js'	=> [
							'bootbox/bootbox.min',
	...
	private $url = [ 'page' => 'setup/hotel/room/','form'=> '.form_crud' ,'autoDataTable' => false];

	public $views = 'Setup.Views.Room.';
		...
		if (Request::ajax())
		{
			...
			switch(Request::get('event'))
			{
				...
				case 'update':
					...
					break;
				case 'saveBulk':
					if ($this->permission->has('update-person-amount')) {
						$data = Request::get('data');
						foreach(jsonToArray($data)[0] as $row) {
							$row = getRawData($row);
							if (getObjectValue($row,'id')) {
								$this->model
									->where('index_id',decode(getObjectValue($row,'id')))
									->update([
										'per_person_amount'=>decimal(Request::get('amount'))
									]);
							}
						}
						SystemLog("Setup","Room","event","update-person-amount",Request::all());
						$response = ['error'=>false,'message'=>'Successfully Save!','table'=> $this->refreshTable()];
					}
				break;
				case 'delete':
				...
				case 'edit':
					...
					break;
				case 'bulkEditAmount':
						$response = view($this->views.'forms.bulk_edit_amount');
					break;
			}
		}
		return $response;
