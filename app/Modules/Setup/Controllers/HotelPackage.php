...
use App\Modules\Setup\Services\HotelPackageServiceProvider as Services;
use App\Modules\Setup\Models\HotelPackage as Model;
use App\Modules\Setup\Models\HotelPackageItems;
...
class HotelPackage extends Controller {
	private $media =
		[
			'Title'=> 'Menu Packages',
			'Description'=> 'Manage',
			'js'		=> ['Setup/Package'],
			'init'		=> ['Package.init()','FN.multipleSelect()'],
			'plugin_js'	=> [
							'bootbox/bootbox.min',
							'select2/select2.min',
	...
	private $url = [ 'page' => 'setup/hotel/package/','form'=> '.form_crud' ];

	public $views = 'Setup.Views.Packages.';
			...
			switch(Request::get('event'))
			{
				...
				case 'delete':
					if ($this->permission->has('delete')) {
						$this->model->where('index_id',decode(Request::get('key')))->update(['is_deleted'=>1]);
				...
				case 'show':
				$response = ['error'=>false,
								'data'=> view($this->views.'tables.package')->render(),
							];
				break;
			}
		}
	...
	private function initializer()
	{
		...
		$this->items = new HotelPackageItems;
		$this->product = new ProductModel;
		$this->permission = new Permission('hotel-package');
	}
}